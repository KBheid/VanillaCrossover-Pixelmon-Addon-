...
public class StoragePokemonInteraction {
    ...
    private static void trySetPalette(Pokemon pokemon) {
        ...
        if (pokemon.hasPalette(paletteToTry)) {
            ...
            PixelBlend.LOGGER.info("Tried to set a storage mon's palette, but the palette was not found (no palette change occurred - this is not an issue!)");
            PixelBlend.LOGGER.info("See this Pokemon's definition if it should have a custom palette: " + pokemon.getSpecies() + ":" + paletteToTry);
        }
    }
