    ...
    static void onServerStopping(FMLServerStoppingEvent event) {
        for (PlayerPokemonStorage storage : cachedStorages.values()) {
            try {
                ...
                VanillaCrossover.LOGGER.error("Unable to open file: " + storage.saveFile.getAbsolutePath() + " to save pokemon storage data.");
                e.printStackTrace();
            }
        }
    }
        ...
        if (cachedStorages.containsKey(player.getUUID())) {
            ...
            if (nbtFile.exists()) {
                try (DataInputStream dataStream = new DataInputStream(Files.newInputStream(nbtFile.toPath()))) {
                    ...
                    VanillaCrossover.LOGGER.error("Unable to open file: " + nbtFile.getAbsolutePath() + " to load pokemon storage data.");
                    e.printStackTrace();
                }
            }
            ...
            // Otherwise, create it and cache it
                ...
                try {
                    ...
                    VanillaCrossover.LOGGER.error("Unable to create file: " + nbtFile.getAbsolutePath() + " to save pokemon storage data.");
                    e.printStackTrace();
                }
            }
        }
